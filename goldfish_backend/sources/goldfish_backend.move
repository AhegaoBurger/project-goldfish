...
module 0xcaffee::file_registry {
    ...
    struct FileIdAdded has copy, drop {
        user: address,
        file_id: ID
    }

    /// Event emitted when a file ID is removed for a user.
    struct FileIdRemoved has copy, drop {
        user: address,
        file_id: ID
    }
    ...
    public entry fun add_file_id(
        ...
        if (table::contains(®istry.user_files, sender)) {
    ...
    public entry fun remove_file_id(
        ...
        assert!(table::contains(®istry.user_files, sender), EUserNotFound);
    ...
    public fun get_file_ids(registry: &FileRegistry, user_address: address): vector<ID> {
        if (table::contains(®istry.user_files, user_address)) {
     ...
     public fun get_file_id_by_index(registry: &FileRegistry, user_address: address, index: u64): option::Option<ID> {
        if (table::contains(®istry.user_files, user_address)) {
            let user_files_vec = table::borrow(®istry.user_files, user_address);
